...
- **Wi-Fi advanced params only go live after a miniport reset.** Intel reads `*LsoV2IPv4/6`, `InterruptModeration`, `PowerSavingMode`, `TxPowerLevel`, `MIMOPowerSaveMode`, etc. at driver init — a raw registry write persists and read-back-verifies but the *running* driver keeps the old values until the adapter is power-cycled (Device Manager's Advanced tab does this automatically; we don't). This silently no-ops the anti-jitter tweaks. `wifi_optimizer.apply()` therefore flags `_requires_restart` (set when a verified write actually *changed* the value — old `backup[value_name]` ≠ target) and captures `_driver_desc`. `MainWindow._maybe_restart_wifi_adapter(backup)` then spawns `_WifiRestartWorker` (async, off the Qt thread) → `wifi_optimizer.restart_adapter(driver_desc)` runs `Restart-NetAdapter -InterfaceDescription <desc> -Confirm:$false` (regex-match fallback when no desc), dropping Wi-Fi ~5-10 s. It is called from **every** real Wi-Fi apply path: `_on_wifi_apply`, `_activate_game_mode`, `_apply_recommendation`, and `_on_disable_setting`. Change-detection means identical re-applies (and post-reboot, where the driver already read the registry at boot) skip the restart — no needless disconnect. `_on_wifi_restart_done(ok, error)` toasts success / "disable-enable in Device Manager or reboot" on failure
...
- Stable-ping Game Mode (`core/stable_ping_policy.py`) is **Wi-Fi-only for both games**. `build_game_mode_plan` returns only a `wifi` section — Valorant gets the conservative 4-key subset (`VALORANT_WIFI_ENABLED_KEYS`), CS2 gets the wider 6-key subset (`CS2_WIFI_ENABLED_KEYS` = +`disable_mimo_power_save` +`disable_bss_scan`). FPS Booster, Optimizer (background killer / TCP / DNS / `system_tweaks`), and per-app DSCP QoS are **never** auto-applied for stable-ping games — they were a recurring source of stutter/thermal-throttle/confusion and stay manual (tabs + saved profiles). The MainWindow DSCP plumbing (`_apply_game_mode_dscp`, `_game_mode_dscp_policy`, deactivate teardown) remains as defensive cleanup but is no longer reached by the stable-ping plan. The `CS2 Stable Ping` seed profile in `profile_manager.py` is likewise Wi-Fi-only
